...
n8n Schedule Trigger
  ...
  → npm run coach-cycle
  → Supabase
  → Discord (only after a new game is analyzed)
```
...
The repository must contain the workflow and the required GitHub repository secrets documented in the main README. Add `DISCORD_WEBHOOK_URL` only when Discord reports are desired. The workflow has concurrency protection so two scheduled runs cannot analyze the same next game simultaneously.
